...
func (s *Service) Init(prof *debug.Profile) {
	prof.Fork("initial gRPC client for (app_luexu)")
	ae.PanicOn(s.initGRPCClient())

	if ok := s.waitForReady(); !ok {
		ae.PanicF("gRPC client health check failed for (app_luexu: %s) check health", s.target)
	}
...
func (s *Service) initGRPCClient() *ae.Error {
	...
	if err != nil {
		return ae.NewF(ae.GatewayTimeout, "new gRPC client for (app_luexu: %s) error: %s", addr, err.Error())
	}
	s.conn = conn
	s.target = addr
	return nil
...
func (s *Service) watchTerminate() {
	...
	alog.Stopf("gRPC client for (app_luexu: %s)", s.target)
	if s.conn != nil {
		s.conn.Close()
	}
}
