...
func (s *Service) Init(prof *debug.Profile) {
	prof.Fork("initial gRPC client for (aajs)")
	ae.PanicOn(s.initGRPCClient())

	if ok := s.waitForReady(); !ok {
		ae.PanicF("gRPC client health check failed for (aajs: %s) check health", s.target)
	}
...
func (s *Service) initGRPCClient() *ae.Error {
	serviceName := s.app.Config.GetString("svc_aajs.grpc_service_name", "aajs")
	...
	if err != nil {
		return ae.NewF(ae.GatewayTimeout, "new gRPC client for (aajs: %s) error: %s", addr, err.Error())
	}
	s.conn = conn
	s.target = addr
	return nil
...
func (s *Service) watchTerminate() {
	...
	alog.Stopf("gRPC client for (aajs: %s)", s.target)
	if s.conn != nil {
		s.conn.Close()
	}
}
