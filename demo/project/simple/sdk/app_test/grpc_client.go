...
func (s *Service) Init(prof *debug.Profile) {
	prof.Fork("initial gRPC client for (app_test)")
	ae.PanicOn(s.initGRPCClient())

	if ok := s.waitForReady(); !ok {
		ae.PanicF("gRPC client health check failed for (app_test: %s) check health", s.target)
	}
...
func (s *Service) initGRPCClient() *ae.Error {
	serviceName := s.app.Config.GetString("app_test.grpc_service_name", "app_test")
	...
	if err != nil {
		return ae.NewF(ae.GatewayTimeout, "new gRPC client for (app_test: %s) error: %s", addr, err.Error())
	}
	s.conn = conn
	s.target = addr
	return nil
...
func (s *Service) watchTerminate() {
	...
	alog.Stopf("gRPC client for (app_test: %s)", s.target)
	if s.conn != nil {
		s.conn.Close()
	}
}
